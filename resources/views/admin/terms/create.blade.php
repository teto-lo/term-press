...
@section('content')
    <h1 class="text-3xl font-bold mb-6">記事作成</h1>

    <form action="{{ route('admin.terms.store') }}" method="POST" class="bg-white shadow rounded-lg p-6">
        @csrf
        ...
        <div class="mb-4">
            ...
            <input type="text" name="short_description" value="{{ old('short_description') }}" maxlength="100" required
        ...
        <div class="mb-4">
            ...
            <textarea name="example" rows="3"
                class="w-full border rounded px-3 py-2 @error('example') border-red-500 @enderror">{{ old('example') }}</textarea>
            @error('example')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="custom_ad_code" rows="5"
                class="w-full border rounded px-3 py-2 font-mono text-sm @error('custom_ad_code') border-red-500 @enderror">{{ old('custom_ad_code') }}</textarea>
            @error('custom_ad_code')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
                ...
                @foreach ($sceneTerms as $sceneTerm)
                    <label class="flex items-center gap-2 text-sm">
                        <input type="checkbox"
                            name="scene_term_ids[]"
                            value="{{ $sceneTerm->id }}"
                            {{ in_array($sceneTerm->id, old('scene_term_ids', [])) ? 'checked' : '' }}>
                        {{ $sceneTerm->name }}
                    </label>
                @endforeach
            </div>
